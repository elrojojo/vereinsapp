function Termine_PersonenkreisBeschraenkenErstellen($btn, liste) {
    ...
    const $formular = $btn
        .parents(".personenkreis_beschraenken_definition")
        .first();
    const filtern_liste = $btn
        .parents("[data-filtern_liste]")
        .first()
        .attr("data-filtern_liste");
    ...
    $formular.find(".personenkreis_beschraenken_wert").each(function () {
        ...
        if ($personenkreis_beschraenken_wert.val()) {
            const operator =
                $personenkreis_beschraenken_wert.attr("data-operator");
            let wert = $personenkreis_beschraenken_wert.val();

            if (
                wert &&
                !Number.isNaN(Number(wert)) &&
                typeof wert !== "boolean"
            )
                wert = Number(wert);
            if (
                typeof EIGENSCHAFTEN[LISTEN[filtern_liste].controller][
                    filtern_liste
                ][eigenschaft] !== "undefined" &&
                EIGENSCHAFTEN[LISTEN[filtern_liste].controller][filtern_liste][
                    eigenschaft
                ]["typ"] == "zeitpunkt"
            )
    ...
    if (filtern_eigenschaft.length == 1)
        filtern_eigenschaft_knoten = filtern_eigenschaft[0];
    else
    ...
    const filtern_mitglieder =
        LISTEN[liste].tabelle[element_id].filtern_mitglieder;
    if (filtern_mitglieder.length == 0)
        filtern_mitglieder.push(filtern_eigenschaft_knoten);
    else {
