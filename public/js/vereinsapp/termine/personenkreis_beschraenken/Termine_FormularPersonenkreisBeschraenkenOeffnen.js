function Termine_FormularPersonenkreisBeschraenkenOeffnen($formular, event, liste) {
    ...
    $.each(FILTERBARE_EIGENSCHAFTEN[filtern_liste], function (index, eigenschaft) {
        const EIGENSCHAFT = EIGENSCHAFTEN[G.LISTEN[filtern_liste].controller][filtern_liste][eigenschaft];
        const typ = EIGENSCHAFT.typ;
        const beschriftung = EIGENSCHAFT.beschriftung;
        const $neue_personenkreis_beschraenken_definition = PERSONENKREIS_BESCHRAENKEN.$blanko_personenkreis_beschraenken_definition[typ]
            .clone()
        ...
        if (typ == "vorgegebene_werte") {
            ...
            $.each(VORGEGEBENE_WERTE[filtern_liste][eigenschaft], function (wert, eigenschaften) {
                $('<option value="' + wert + '">' + eigenschaften.beschriftung + "</option>").appendTo(
                    $neue_personenkreis_beschraenken_definition.find(".personenkreis_beschraenken_wert")
                );
            });
        }
