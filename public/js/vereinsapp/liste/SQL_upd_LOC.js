...
$(document).on( 'SQL_upd_LOC', async function( event, schleife, liste ) { if( typeof schleife === 'undefined' ) schleife = false; const LISTE = LISTEN[ liste ];
    $.ajaxQueue( {
        url: BASE_URL+'/'+LISTE.controller+'/ajax_'+liste,
        method: 'post',
        ...
        error: function(xhr) { console.log( 'FEHLER SQL_upd_LOC_'+liste+': ' + xhr.status + ' ' + xhr.statusText); },
        complete: function() {
            $(document).trigger( 'LOC_upd_VAR', [ liste ] );    // impliziert auch ein $(document).trigger( 'VAR_upd_DOM', [ liste ] );
            if( schleife ) setTimeout( function() { $(document).trigger( 'SQL_upd_LOC', [ true, liste ] ) }, AJAX_REFRESH_INTERVALL*1000 );
        },
    } );
} );