function LOC_upd_VAR(liste) {
    ...
    const LOC_tabelle = localStorage.getItem("vereinsapp_" + liste + "_tabelle");
    if (LOC_tabelle !== null && LOC_tabelle != "undefined") {
        $.each(JSON.parse(LOC_tabelle), function () {
            const element = this;
    ...
    LISTE.sortieren = new Array();
    const LOC_sortieren = localStorage.getItem("vereinsapp_" + liste + "_sortieren");
    if (LOC_sortieren !== null && LOC_sortieren != "undefined") LISTE.sortieren = JSON.parse(LOC_sortieren);

    LISTE.filtern = new Array();
    const LOC_filtern = localStorage.getItem("vereinsapp_" + liste + "_filtern");
    if (LOC_filtern !== null && LOC_filtern != "undefined") LISTE.filtern = JSON.parse(LOC_filtern);
