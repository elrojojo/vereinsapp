function Liste_Filtern2$Filtern(filtern, liste) {
    ...
    $.each(filtern, function (index, knoten) {
        if ("verknuepfung" in knoten) {
            ...
            $.each(Liste_Filtern2$Filtern(knoten.filtern, liste), function (index, $filtern) {
            ...
            let data_wert = wert;
            if (EIGENSCHAFTEN[LISTE.controller][liste][eigenschaft].typ == "zeitpunkt") data_wert = wert.toFormat(SQL_DATETIME);
