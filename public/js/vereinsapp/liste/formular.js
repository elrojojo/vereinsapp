$(document).ready( function() {
    ...
    $('.formular').on('show.bs.modal', function ( event ) { const $formular = $(this); const $btn_oeffnend = $(event.relatedTarget); const aktion = $btn_oeffnend.attr('data-aktion');
        
        let element = $btn_oeffnend.attr('data-element'); let element_id = $btn_oeffnend.attr('data-element_id'); let liste = $btn_oeffnend.attr('data-liste');
        ...
        if( typeof liste === 'undefined' ) $.each( LISTEN, function( liste_each, LISTE_each ) { if( element == LISTE_each.element ) liste = liste_each; } );
        ...
        $formular.find('.is-invalid').removeClass('is-invalid'); $formular.find('.is-valid').removeClass('is-valid');

        if( aktion == 'erstellen' || aktion == 'duplizieren' || aktion == 'aendern' ) {
            $formular.find('.eigenschaft').each( function() { const $eigenschaft = $(this); const eigenschaft = $eigenschaft.attr('data-eigenschaft'); let val;
                if( typeof element_id !== 'undefined' ) val = LISTE.tabelle[ element_id ][ eigenschaft ]; else val = EIGENSCHAFTEN[ LISTE.controller ][ liste ][ eigenschaft ].standard;
                if( $eigenschaft.attr('type') == 'date' ) { if( typeof val !== 'object' ) val = DateTime.fromFormat( val, SQL_DATETIME ); val = val.toISODate(); }
                else if( $eigenschaft.attr('type') == 'time' ) { if( typeof val !== 'object' ) val = DateTime.fromFormat( val, SQL_DATETIME ); val = val.toISOTime( { includeOffset: false, suppressSeconds: true, suppressMilliseconds: true } ); }
                $eigenschaft.val( val ).change();
            } );
        } else
        if( aktion == 'loeschen' ) {
            $formular.find('.beschriftung').html( $( '.element[data-element="'+element+'"][data-element_id="'+element_id+'"]' ).find('.beschriftung').html() ); // Für element_loeschen (modal)
        } else
        if( aktion == 'sortieren' ) {
        ...
        if( typeof element !== 'undefined' ) $btns.attr( 'data-element', element ); else $btns.removeAttr('data-element');
        if( typeof aktion !== 'undefined' ) $btns.attr( 'data-aktion', aktion ); else $btns.removeAttr('data-aktion');
        if( typeof element_id !== 'undefined' && aktion != 'duplizieren' ) $btns.attr( 'data-element_id', element_id ); else $btns.removeAttr('data-element_id');
    } );
