function Mitglieder_PermissionsAktualisieren( $permissions, liste ) {
    ...
    $.each( PERMISSIONS, function( permission, beschriftung) {
        ...
        if( ! $permissions.find( '[value="'+permission+'"]' ).parents('.permission').first().exists() ) {

            const $neue_permission = LISTEN[ liste ].$blanko_permission.clone().removeClass('blanko invisible').addClass('permission');
