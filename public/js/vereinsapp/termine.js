...
$(document).ready( function() {
  ...
  //LISTEN UND ELEMENTE IM DOM AKTUALISIEREN
      ...
      $element.find('.zusagen, .absagen').each( function() {
        ...
        if( $btn_rueckmelden.hasClass('zusagen') ) {
          if( ich_rueckmeldung_id !== null && LISTEN.rueckmeldungen.tabelle[ ich_rueckmeldung_id ].status >= 1 ) {
            ...
            if( typeof LISTEN.rueckmeldungen.tabelle[ ich_rueckmeldung_id ].bemerkung !== 'undefined' && LISTEN.rueckmeldungen.tabelle[ ich_rueckmeldung_id ].bemerkung != '' ) $btn_rueckmeldung_detaillieren.removeClass('btn-outline-success').addClass('btn-success');
          }
          else {
          ...
          if( ich_rueckmeldung_id !== null && LISTEN.rueckmeldungen.tabelle[ ich_rueckmeldung_id ].status == 0 ) {
            ...
            $btn_rueckmeldung_detaillieren.removeClass('invisible').attr( 'data-element_id', ich_rueckmeldung_id );
            if( typeof LISTEN.rueckmeldungen.tabelle[ ich_rueckmeldung_id ].bemerkung !== 'undefined' && LISTEN.rueckmeldungen.tabelle[ ich_rueckmeldung_id ].bemerkung != '' ) $btn_rueckmeldung_detaillieren.removeClass('btn-outline-danger').addClass('btn-danger');
          }
          else {
