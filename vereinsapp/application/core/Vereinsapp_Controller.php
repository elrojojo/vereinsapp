...
class Vereinsapp_Controller extends CI_Controller {
  ...
  public function __construct() {
    ...
    defined('HEAD_SCRIPT') OR define( 'HEAD_SCRIPT', array( 
      //'jquery-x.slim.min.js' => array( 'src' => 'https://code.jquery.com/jquery-3.5.1.slim.min.js', 'integrity' => 'sha384-DfXdz2htPH0lsSSs5nCTpuj/zy4C+OGpamoFVy38MVBnE+IbbVYUew+OrCXaRkfj', 'crossorigin' => 'anonymous', ),
      ...
      'jquery.ui.touch-punch.js' => array( 'src' => '/vereinsapp_storage/js/jquery-ui-touch-punch/jquery.ui.touch-punch.min.js', ),
    ...
    defined('BASE_URL') OR define( 'BASE_URL', $this->config->item('base_url') ); // braucht es das?
    ...
    $scrollpos = 0; if( explode( '/', $this->session->absprung )[0] == CONTROLLER ) $scrollpos = $this->input->cookie( 'vereinsapp_scrollpos' ); defined('SCROLLPOS') OR define( 'SCROLLPOS', $scrollpos );  // Wenn der vorherige Controller nicht dem jetzigen entspricht, dann setz die scrollpol auf 0
    ...
    if( array_key_exists( CONTROLLER, CONTROLLER_INT ) AND
        ...
        !$this->login_model->dauerhaft_angemeldet( $this->input->cookie('vereinsapp_dauerhaft_angemeldet_identifier'), $this->input->cookie('vereinsapp_dauerhaft_angemeldet_securitytoken') )
    ) redirect(LOGIN);
    ...
    defined('COOKIES_RICHTLINIE_DATUM') OR define( 'COOKIES_RICHTLINIE_DATUM', 20210629 );  // Datum der letzten Aktualisierung der Richtlinien
    if( !boolval($this->input->cookie( 'vereinsapp_cookies_'.strval(COOKIES_RICHTLINIE_DATUM) )) ) $this->data['modals_anzeigen_liste']['cookies_richtlinie'] = array( 'view' => $this->load->view('cookies/richtlinie', $this->data, TRUE ) );
    ...
    //status_feuern( 'danger','ID = '.$this->input->cookie('vereinsapp_dauerhaft_angemeldet_identifier') );
