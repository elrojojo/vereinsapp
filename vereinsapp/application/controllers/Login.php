...
class Login extends Vereinsapp_Controller {
  ...
  public function logout() {
    if( $this->session->ich_id AND session_destroy() ) {  // Logout ist erfolgreich
      ...
      //delete_cookie( 'vereinsapp_cookies_'.strval(COOKIES_RICHTLINIE_DATUM) );  // Java-Script in view cookies/richtlinie- -> <button> (path=/ wurde entfernt, weil evtl. problematisch auf bestimmten Browsern)
      //delete_cookie( 'vereinsapp_passwort_vergeben' );  // Java-Script in view einstellungen/passwort_vergeben_modal -> <button>
      delete_cookie( 'vereinsapp_scrollpos' );  // Java-Script in view templates/header- -> <body>
