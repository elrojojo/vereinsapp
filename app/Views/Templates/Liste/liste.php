...
?>>
    ...
    if( array_key_exists( 'modal', $liste ) OR array_key_exists( 'link', $liste ) ) { ?> role="button"<?php } ?>>
        ...
        <div <?php if( array_key_exists( 'beschriftung', $liste ) AND array_key_exists( 'h5', $liste['beschriftung'] ) AND $liste['beschriftung']['h5'] ) echo 'class="h5"'; ?>>
            ...
            <?php if( array_key_exists( 'werkzeugkasten', $liste ) ) { ?><i class="bi bi-three-dots-vertical text-primary float-end stretched-link-unwirksam" role="button" data-bs-toggle="collapse" data-bs-target="#werkzeugkasten_"></i><?php } ?>
        ...
        <?php if( array_key_exists( 'werkzeugkasten', $liste ) ) { ?><div class="collapse <?php if( array_key_exists( 'beschriftung', $liste ) AND array_key_exists( 'h5', $liste['beschriftung'] ) AND $liste['beschriftung']['h5'] ) { ?>h5 <?php } ?>stretched-link-unwirksam" id="werkzeugkasten_"><span data-bs-toggle="collapse" data-bs-target="#werkzeugkasten_">
            <?php foreach( $liste['werkzeugkasten'] as $aktion => $werkzeug):
            ?><i class="bi bi-<?= SYMBOLE[ $aktion ]['bootstrap']; ?> text-<?php if( array_key_exists( 'farbe', $werkzeug ) ) echo $werkzeug['farbe']; else echo 'primary'; ?> float-end ms-2" data-bs-toggle="modal" data-bs-target="<?= $werkzeug['modal_id']; ?>" data-aktion="<?= $aktion; ?>" role="button"></i>
            <?php endforeach; ?>
        </span></div><?php } ?>
