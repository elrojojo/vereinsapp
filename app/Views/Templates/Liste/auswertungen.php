<?php if( array_key_exists( 'werkzeugkasten', $auswertungen ) ) { ?><div class="werkzeugkasten h5 text-end">
    <?php foreach( $auswertungen['werkzeugkasten'] as $aktion => $werkzeug) { ?><button class="btn werkzeug text-<?php
        ...
        ?>" data-liste="<?= $auswertungen['liste']['liste']; ?>" data-aktion="<?= $aktion; ?>" data-title="<?= $werkzeug['title']; ?>" data-instanz="<?= $auswertungen['id']; ?>"<?php
